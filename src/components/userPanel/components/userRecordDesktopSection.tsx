...
function Items(props: { recordRef: any, currentItems: any, deleteUserRecord: Function }) {
...
const UserRecordList = (props: { recordRef: any, userRecord: IRecord[], isMobile: boolean, deleteUserRecord: Function }) => {
    const { userRecord, isMobile, deleteUserRecord, recordRef } = props;
    return <>
        <div className=" w-full h-full flex flex-col overfloat-y-auto">
            {userRecord.length > 0 && <Paginate
                pageClassName="  "
...
const TitleText = (props: { userRecord: IRecord[] }) => {
    ...
    return <div className=" mb-2 text-black dark:text-white text-[24px] md:text-[30px] font-bold text-center">
        {userRecord.length > 0 ? "User Record" : " A tool to record your pokemon."}

    </div>
}
const UserRecordDesktopSection = (props: { recordRef: any, userRecord: IRecord[], isMobile: boolean, deleteUserRecord: Function }) => {
    const { userRecord, isMobile, deleteUserRecord, recordRef } = props
