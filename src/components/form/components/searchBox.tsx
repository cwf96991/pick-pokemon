...
import { MobileExtraFilterButton, ExtraFilterWidget, ExtraFilterWidgetMobile } from "./extraFilter"
...
const SearchBox = (props: { setSearchQuery: Function, turnstoneRef: any, toggleFilterParams: Function, isSelectedFilter: Function, filterTagWidget: ReactElement, getSizebyParam: Function, getSelectedCountByType :Function}) => {
    const { setSearchQuery, turnstoneRef, toggleFilterParams, isSelectedFilter, filterTagWidget, getSizebyParam, getSelectedCountByType } = props
    const { isOpen, toggleDrawer } = useDrawer()
    const queryString = useRef("")
    const { pokemons, abilities, colors, eggGroups, types, natures, growthRates, habitats, shapes } = useGetPokemons()
    ...
    const Item = (data: {
        ...
        return (
            ...
            <div className="flex justify-between px-5 py-2 cursor-pointer items-center">
                ...
                {isSelected && <input type="checkbox" readOnly checked={true} className="checkbox checkbox-warning" />}
            </div>
        )
    }
