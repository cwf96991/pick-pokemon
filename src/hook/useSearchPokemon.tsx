...
const useSearchPokemon = (search: string) => {
  ...
  const [searchPokemons, setSearchPokemons] = useState<ResponseAPI[]>(
    [] as ResponseAPI[]
  );
  ...
  const searchPokemonByFilter = async (type: string, query: string) => {
    ...
    try {
      ...
      const index = filterParams?.findIndex(
        (param: IFilterParams) => param.type === type && param.query === query
      ) as number;
    ...
    try {
      ...
      for (let index = 0; index < filterParams.length; index++) {
        ...
        if (param.type === "pokemon") {
          searchPokemon(param.query).then((data) => {
            const index2 = getSearchPokemonsIndexByName(data?.name as string);
            if (index2 === -1) {
              searchPokemons?.unshift(data as ResponseAPI);
              const set = new Set([...(searchPokemons as ResponseAPI[])]);
  ...
  async function toggleFilterParams(type: string, query: string) {
    ...
    if (type === "name" || type === "pokemon") {
      ...
      const index = filterParmsRef.current.findIndex(
        (parmaData: { type: string; query: string }) =>
          parmaData.type === type && parmaData.query === query
      );
      const filterParams = filterParmsRef.current;
      let isAdd = true;
      if (index !== -1) {
  ...
  function getSizebyParam(query: string) {
    const filterParams = filterParmsRef.current;
    const index = filterParams?.findIndex(
      (param: IFilterParams) => param.query === query
    ) as number;
    if (index !== -1) {
  ...
  function getTotalPokemonFromFilter() {
    const filterParams = filterParmsRef.current;
    let count = 0;
    filterParams.forEach((param: IFilterParams) => {
  ...
  function getSelectedCountByType(type: string): number {
    const filterParams = filterParmsRef.current;
    return filterParams?.filter((param: IFilterParams) => param.type === type)
      .length;
  }
  function getSearchPokemonsIndexByName(name: string) {
    if (localStorage.searchedPokemons) {
      const localSearchedPokemons = JSON.parse(
        localStorage.searchedPokemons ?? ""
      );
      return localSearchedPokemons?.findIndex(
        (pokemon: { name: string }) => pokemon.name === name
      ) as number;
    } else {
      return searchPokemons?.findIndex(
        (pokemon: { name: string }) => pokemon.name === name
      ) as number;
    }
  }
  ...
  async function loadSearchedPokemons() {
    if (localStorage.searchedPokemons && searchPokemons?.length === 0) {
      ...
      for (let index = 0; index < localSearchedPokemons.length; index++) {
        ...
        const index2 = searchPokemons?.findIndex(
          (pokemon: { name: string }) => pokemon.name === query
        ) as number;
        if (index2 === -1) {
          searchPokemons?.unshift(data as ResponseAPI);
        }
